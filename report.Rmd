...
skew1c <- lm(post.trt ~ pct_10 + I(pct_10^2) + I(pct_10^3), data = mdf5[mdf5$shift == -0.6,]) # cubic terms now have higher pvals
...
# colnames(skew1_tbl) <- c('Skew', 'Intercept', 'P', 'P^2', 'P^3')  
...
\[
  \makebox[\linewidth]{$Y_j = \beta_0 + \beta_{1j}p + \beta_{2j}p^2 + \beta_{3j}p^3 + \beta_{4j}g + \beta_{5j}p_j*g_j + \epsilon$}
\]  
...
# colnames(skew2_tbl) <- c('skew', 'int', 'pct_lin', 'pct_quad', 'pct_cub', 'gran5', 'gran6', 'gran7', 'gran11', 'pctXgran5', 'pctXgran6', 'pctXgran7', 'pctXgran11')
...
```{r}
#### Plot of effect of gran for various pct levels, grouped color-wise by shift (a.k.a. the model where the effect came from)
...
## PCT = 0.3
pct30 <- ggplot(data = skew2_tbl_pivot %>% filter(skew >= 0),  # only looking at one side of the skew
...
## PCT = 0.2
pct20 <- ggplot(data = skew2_tbl_pivot %>% filter(skew >= 0),  # only looking at one side of the skew
...
## PCT = 0.1
pct10 <- ggplot(data = skew2_tbl_pivot %>% filter(skew >= 0),  # only looking at one side of the skew
...
## PCT = 0
pct0 <- ggplot(data = skew2_tbl_pivot %>% filter(skew >= 0),  # only looking at one side of the skew
...
# Negative side
## PCT = 0.4
pct40neg <- ggplot(data = skew2_tbl_pivot %>% filter(skew <= 0),  # only looking at one side of the skew
                   aes(x = as.integer(gran), y = effect_pct40, color = as.factor(skew))) + 
  geom_point(size = 5, alpha = 0.4, stroke = 0) +
  ylab(str_wrap('Effect of Granularity with 40 Percent Contamination', width = 30)) + xlab('Granularity') +
...
## PCT = 0.3
pct30neg <- ggplot(data = skew2_tbl_pivot %>% filter(skew <= 0),  # only looking at one side of the skew
                   aes(x = as.integer(gran), y = effect_pct30, color = as.factor(skew))) + 
  geom_point(size = 5, alpha = 0.4, stroke = 0) +
  ylab(str_wrap('Effect of Granularity with 30 Percent Contamination', width = 30)) + xlab('Granularity') +
  scale_colour_manual(name="Skew", values = rev(c('orangered', 'coral', 'orange', 'darkgoldenrod1', 'gold1', 'springgreen4'))) +
...
## PCT = 0.2
pct20neg <- ggplot(data = skew2_tbl_pivot %>% filter(skew <= 0),  # only looking at one side of the skew
                   aes(x = as.integer(gran), y = effect_pct20, color = as.factor(skew))) + 
  geom_point(size = 5, alpha = 0.4, stroke = 0) +
  ylab(str_wrap('Effect of Granularity with 20 Percent Contamination', width = 30)) + xlab('Granularity') +
  scale_colour_manual(name="Skew", values = rev(c('orangered', 'coral', 'orange', 'darkgoldenrod1', 'gold1', 'springgreen4'))) +
...
## PCT = 0.1
pct10neg <- ggplot(data = skew2_tbl_pivot %>% filter(skew <= 0),  # only looking at one side of the skew
                   aes(x = as.integer(gran), y = effect_pct10, color = as.factor(skew))) + 
  geom_point(size = 5, alpha = 0.4, stroke = 0) +
  ylab(str_wrap('Effect of Granularity with 10 Percent Contamination', width = 30)) + xlab('Granularity') +
  scale_colour_manual(name="Skew", values = rev(c('orangered', 'coral', 'orange', 'darkgoldenrod1', 'gold1', 'springgreen4'))) +
...
## PCT = 0
pct0neg <- ggplot(data = skew2_tbl_pivot %>% filter(skew <= 0),  # only looking at one side of the skew
                  aes(x = as.integer(gran), y = effect_pct0, color = as.factor(skew))) + 
  geom_point(size = 5, alpha = 0.4, stroke = 0) +
  ylab(str_wrap('Effect of Granularity with 0 Percent Contamination', width = 30)) + xlab('Granularity') +
  scale_colour_manual(name="Skew", values = rev(c('orangered', 'coral', 'orange', 'darkgoldenrod1', 'gold1', 'springgreen4'))) +
...
```{r, results = "asis"}
names(pm1$coefficients) <- c('(Intercept)', '10\\% Contamination', 'Skewness (by 0.2)', 'Skewness (by 0.2)$^2$')
pm1tbl <- summary(pm1)$coefficients %>% round(4)
pm1tbl[pm1tbl == 0] <- '<2e-16'
print(xtable(pm1tbl), sanitize.text.function = function(x) {x})
```
